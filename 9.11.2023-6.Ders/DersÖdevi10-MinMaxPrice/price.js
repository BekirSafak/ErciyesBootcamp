...
function filterPrice() {
    ...
    let searched = products.filter(product => product.unitPrice > minPrice && product.unitPrice < maxPrice);

    searched.forEach(element => {
        let filterLi = document.createElement("li");
        if (element.unitsInStock == 0) {
            filterLi.style.color = "#3700ff"
        }
        filterLi.innerHTML = "isim : " + element.name + " / " + "Fiyat : " + element.unitPrice + " / " + "Stok : " + element.unitsInStock;
        filterList.appendChild(filterLi)
    });
