...
let orderDateList = document.querySelector('.orderDateList');
let listLength = [];

let dateOrder = orders.filter(order => order.orderDate.includes("1996-12"));
dateOrder.forEach(element => {

    let orderLi = document.createElement('li');
    orderLi.style.marginBottom = "16px"
    orderLi.innerHTML = moment(element.orderDate).format('D MMMM YYYY h:mm A');;

    orderList.appendChild(orderLi)

    let countOrder = document.querySelector('.countOrder');
    let listCount = listLength.push(orderLi)
    countOrder.innerHTML = " / " + listCount + " Adet Sipariş Var!";
});
...
function orderShow() {
    ...
    orders.forEach(element => {
        ...
        let required = new Date(element.requiredDate);
        let shipped = new Date(element.shippedDate);

        let formatShipped = moment(shipped).format("D")
        let formatRequired = moment(required).format("D")


        if (formatShipped - formatRequired > 5 && formatShipped - formatRequired < 10) {
            let orderDateList = document.querySelector('.orderDateList');
            let orderDateLi = document.createElement('li')
            orderDateLi.style.marginBottom = "16px"

            orderDateLi.innerHTML = element.shipName;

            orderDateList.appendChild(orderDateLi)
            console.log("Kaç tane?")

            let countShip = document.querySelector('.countShip');
            let listShip = listLength2.push(orderDateLi);
            countShip.innerHTML = " / " + listShip + " Adet Sipariş Var!";
        }
